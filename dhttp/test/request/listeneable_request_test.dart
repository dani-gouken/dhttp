...
void main()
{
 test("the progress updates", () async{
  ...
  ListeneableHttpRequest request = new ListeneableHttpRequest("POST", Uri.https("google.fr", ""),progressListener: listener);
 ...
 test("the it can be cancelled", () async{
  ...
  ListeneableHttpRequest request = new ListeneableHttpRequest("POST", Uri.https("google.fr", ""),cancellationToken: cancellationToken);
  
  await client.send(request);

  expect(cancellationToken.cancelled,1);
 }); 
}